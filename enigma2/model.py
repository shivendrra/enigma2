...
class SplineEdge(nn.Module):
  ...
  def forward(self, x:torch.tensor):
    ...
    # Compute bin indices
    bins = torch.bucketize(x_clamped, self.knots) - 1
...
class LatentHead(nn.Module):
  def __init__(self, head_size, d_model, block_size, latent_dim, dropout, device, bias=False):
    ...
    # for values: similar decomposing the proj into two parts: wa_key, wb_key
  ...
  def forward(self, x: torch.tensor):
    ...
    # expanding the latent represtations to full key & values
    ...
    scores = torch.matmul(w_query, key.transpose(-2, -1)) / (key.shape[-1]**-0.5)

    if T > self.tril.size(0):
      self.tril = torch.tril(torch.ones(T, T, device=self.device))
    scores = scores.masked_fill(self.tril[:T, :T] == 0, float('-inf'))

    scores = self.dropout(F.softmax(scores, dim=-1))  # applying softmax & dropout
...
class MultiHeadLatentAttention(nn.Module):
  def __init__(self, d_model, dropout, n_head, block_size, device, bias=False, latent_dim=None, projection_bins=4):
    ...
    super().__init__()
    self.heads = nn.ModuleList([LatentHead(head_size, d_model, block_size, latent_dim, dropout, device, bias) for _ in range(n_head)])
...
class DecoderBlock(nn.Module):
  def __init__(self, d_model: int, block_size: int, num_bins: int, n_head: int, norm_eps: float, dropout: float, latent_dim: int, projection_bins: int, bias: bool, device: str, n_ff: int = None, ffn_multiplier: int = None):
    super().__init__()
    self.self_att = MultiHeadLatentAttention(n_head=n_head, d_model=d_model, dropout=dropout, block_size=block_size, device=device, latent_dim=latent_dim, bias=bias, projection_bins=projection_bins)
    self.ffwd = FeedForward(d_model, dropout, num_bins, n_ff, ffn_multiplier)
    self.dropout = nn.Dropout(dropout)
    self.norm = RMSNorm(d_model, eps=norm_eps)

  def forward(self, x: torch.tensor):
    x_out = self.self_att(self.norm(x))
...
class Transformer(nn.Module):
  def __init__(self, params: ModelConfig, vocab_size: int, block_size: int):
    ...
    self.decoder = nn.Sequential(*[
      DecoderBlock(d_model=params.d_model,
